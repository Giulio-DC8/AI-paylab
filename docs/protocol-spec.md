...
**Implementation note:** this is not a distributed system or a network protocol,`Buyer` and `Sellers` are not separate processes exchanging messages. In the reference implementation, `negotiate()` is a single function that holds a list of `Seller` objects in memory and calls their `counter_offer()` method directly, passing `best_price` as a plain function argument. The information isolation described in this document (a Seller never learns who the competitor is, or how many other Sellers exist) is a property of *what gets passed as an argument*, not of network-level access control,a language-agnostic reimplementation should preserve this same isolation (e.g. never passing a Seller a reference to the full pool), even without literal message-passing.
